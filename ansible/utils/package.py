...
def script_yml_zookeeper(org_id, org_count):
...
{server}
  ...
  - zookeeper
    ...
    with open(fname, 'w') as ffile:
        ffile.write(template_zookeeper.format(org_id=org_id, zoo_id=zoo_id, server=server))


def script_yml_kafka(org_id, org_count):
...
- hosts: k{org_id}
  ...
  - kafka
    ...
    with open(fname, 'w') as ffile:
        ffile.write(template_kafka.format(org_id=org_id, kafka_id=kafka_id, zookeeper_connect=zookeeper_connect))


def script_yml_fabricca(org_id):
...
- hosts: fabric-ca{org_id}
  ...
  - fabricca
    ...
    with open(fname, 'w') as ffile:
        ffile.write(template_fabricca.format(org_id=org_id))


def script_yml_peer(peer_id, org_id):
...
- hosts: peer{peer_id}.org{org_id}
  ...
  - peer
    ...
    with open(fname, 'w') as ffile:
        ffile.write(template_peer.format(peer_id=peer_id, org_id=org_id))


def script_yml_orderer(org_id):
...
- hosts: orderer{org_id}
  ...
  - orderer
    ...
    with open(fname, 'w') as ffile:
        ffile.write(template_orderer.format(org_id=org_id))


def script_yml_cli(org_id):
...
- hosts: cli{org_id}
  ...
  - cli
    ...
    with open(fname, 'w') as ffile:
        ffile.write(template_cli.format(org_id=org_id))
...
def script_yml(peer_count):
    ...
    org_count = len(peer_count)
    for org in range(0, org_count):
        script_yml_zookeeper(org, org_count)
        script_yml_kafka(org, org_count)
        script_yml_fabricca(org)

        for peer in range(0, peer_count[org]):
            script_yml_peer(peer, org)

        script_yml_orderer(org)
        script_yml_cli(org)
